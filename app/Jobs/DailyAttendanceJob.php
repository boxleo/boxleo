...
use App\Models\Holiday; // Add this
...
class DailyAttendanceJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($isHoliday) {
            Log::info("Today is a holiday: {$date->toDateString()}");
        }
        ...
        foreach ($users as $user) {
            ...
            if (!$hasAttendance) {
                ...
                if ($dayOfWeek === 'Sunday' && !$onLeave && !$isHoliday) {
                    Log::info("It's Sunday and user is not on leave or holiday, skipping...");
                    continue;
                }

                $status = $onLeave ? 'leave' : ($isHoliday ? 'holiday' : 'absent');
                $note = $onLeave
                    ? 'Auto-marked as on leave'
                    : ($isHoliday ? 'Auto-marked as holiday' : 'Auto-marked as absent');
                ...
                Log::info("Created attendance for user {$user->name} ({$user->id}) with status '{$status}'");
            }
        }
